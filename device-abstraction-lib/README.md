...
This library has __DeviceErrors.cpp__ in `src/`. The tests call `to_error` and `operator==`, so every env sets `test_build_src = yes`. HC-LIB.System `.cpp` files arrive through `lib_deps`, which PlatformIO always compiles.
